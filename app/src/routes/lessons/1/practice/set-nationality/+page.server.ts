...
import { setNationality } from '$lib/content/lessons/lesson_1/moves/set_nationality';
...
export async function load() {
	...
	if (!setNationality.eligible(world)) {
		throw error(503, 'No one is missing a nationality.');
	}
	const q = setNationality.prepare(world);
	return {
...
export const actions: Actions = {
	commit: async ({ request }) => {
		...
		if (!entityId || field !== setNationality.field || !isAllowedValue(field, value)) {
