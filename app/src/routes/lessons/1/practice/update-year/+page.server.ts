...
import { updateYear } from '$lib/content/lessons/lesson_1/moves/update_year';
...
export async function load() {
	...
	if (!updateYear.eligible(world)) {
		throw error(503, 'No one has a year set yet.');
	}
	const q = updateYear.prepare(world);
	return {
...
export const actions: Actions = {
	commit: async ({ request }) => {
		...
		if (!entityId || field !== updateYear.field || !isAllowedValue(field, value)) {
